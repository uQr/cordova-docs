...
3.  **You checked in the platforms/android or platforms/ios folder from Windows and are seeing permission errors:** If you are seeing errors that are originating from files in your project's “platforms” folder, you may be running into this problem because you checked in or accidently modified shell scripts in the Android or iOS platforms folder from Windows. These folders are generally not intended to be checked in and unfortunately doing so from Windows will result in a set of shell scripts missing a filesystem “execute bit” required to run them on OSX. (The NTFS filesystem has no such concept.)
